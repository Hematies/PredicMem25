/*
 * Implementation of the Best-Offset (BO) Prefetcher.
 *
 * Originally written for The 2nd Data Prefetching Championship (DPC2) by Pierre Michaud at Inria.
 *
...
public:
  ...
  void invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<int64_t> &pref_addr);
...
void BOPrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<uint64_t> &pref_addr)
{
	...
	if(search_rr(ca_address - offset_to_evaluate))
	{
		...
		cout << "Score of " << to_string(knob::bop_candidates[candidate_ptr]) << ": " << to_string(scores[candidate_ptr]) << "\n"; 
	}
	candidate_ptr++;
...
void BOPrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<int64_t> &pref_offsets)
{
	uint64_t page = address >> LOG2_PAGE_SIZE;
	uint32_t offset = (address >> LOG2_BLOCK_SIZE) & ((1ull << (LOG2_PAGE_SIZE - LOG2_BLOCK_SIZE)) - 1);
	uint64_t ca_address = (address >> LOG2_BLOCK_SIZE);

  auto pref_addr = vector<uint64_t>();
	...
	{
		// int32_t pf_offset = offset + best_offsets[index];
    	pref_offsets.push_back(best_offsets[index] << LOG2_BLOCK_SIZE);
