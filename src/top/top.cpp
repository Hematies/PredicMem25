...
void prefetchWithBOP(block_address_t memoryAddress,
		block_address_t addressesToPrefetch[BOP_PREF_DEGREE]
		){
#pragma HLS INTERFACE ap_fifo port=addressesToPrefetch
#pragma HLS PIPELINE
	static BOPrefetcher<> bop = BOPrefetcher<>();
	bop_offset_t prefetch_deltas[BOP_PREF_DEGREE];
	bop_score_t prefetch_confidences[BOP_PREF_DEGREE];
	uint32_t num_prefetches = 0;
...
#pragma HLS UNROLL
		if (num_prefetches > i) {
			...
			addressesToPrefetch[i] = memoryAddress + prefetch_offset;
		} else {
...
#pragma HLS INTERFACE mode=ap_ctrl_none port=return
#pragma HLS TOP name=SGASP_IP_Recursive
// #pragma HLS INTERFACE mode=m_axi depth=32 max_read_burst_length=16 max_write_burst_length=16 num_read_outstanding=32 num_write_outstanding=32 port=readPort
	#pragma HLS INTERFACE mode=m_axi depth=8 num_read_outstanding=8 port=readPort offset=off
...
void RecursiveSGASPWithAXI_04_01(address_t inputAddress,
		axi_data_t *readPort,
		axi_data_t& prefetchedData,
		bool nop
		){
#pragma HLS INTERFACE mode=ap_ctrl_none port=return
#pragma HLS TOP name=SGASP_IP_Recursive_04_01
// #pragma HLS INTERFACE mode=m_axi depth=32 max_read_burst_length=16 max_write_burst_length=16 num_read_outstanding=32 num_write_outstanding=32 port=readPort
	#pragma HLS INTERFACE mode=m_axi depth=8 num_read_outstanding=8 port=readPort offset=off
...
// #pragma HLS INTERFACE mode=m_axi depth=32 max_read_burst_length=16 max_write_burst_length=16 num_read_outstanding=32 num_write_outstanding=32 port=readPort
	#pragma HLS INTERFACE mode=m_axi depth=8 num_read_outstanding=8 port=readPort offset=off
// #pragma HLS INTERFACE mode=ap_vld port=prefetchValid
#pragma HLS INTERFACE mode=ap_vld port=prefetchedData
#pragma HLS INTERFACE mode=ap_none port=inputAddress
#pragma HLS INTERFACE mode=ap_none port=nop
	...
	if(!nop){
		bop_offset_t prefetch_deltas[BOP_PREF_DEGREE];
		bop_score_t prefetch_confidences[BOP_PREF_DEGREE];
		uint32_t num_prefetches = BOP_PREF_DEGREE;
...
#pragma HLS PIPELINE
	...
	bool performPrefetch = (prefetchAddress >= START_CACHEABLE_MEM_REGION) &&
				(prefetchAddress < END_CACHEABLE_MEM_REGION) &&
				(prefetchAddress != 0);
	prefetchAddress = prefetchAddress >> AXI_DATA_SIZE_BYTES_LOG2;
...
void prefetchWithBSGASPWithDataflow(address_t inputAddress,
	burst_size_t burstSize,
	burst_length_t burstLength,
	address_t& prefetchAddress,
	burst_length_in_words_t& totalBurstLength
...
// #pragma HLS TOP name=prefetchWithBSGASPWithAXI
#pragma HLS INTERFACE mode=ap_ctrl_chain port=return
#pragma HLS INTERFACE mode=m_axi depth=256 latency=5 max_widen_bitwidth=512 num_read_outstanding=32 port=readPort offset=off
#pragma HLS DATAFLOW
...
#pragma HLS INTERFACE mode=ap_ctrl_none port=return
// #pragma HLS TOP name=nextLinePrefetcherWithAXI
// #pragma HLS INTERFACE mode=m_axi depth=32 max_read_burst_length=16 max_write_burst_length=16 num_read_outstanding=32 num_write_outstanding=32 port=readPort
	#pragma HLS INTERFACE mode=m_axi depth=8 num_read_outstanding=8 port=readPort offset=off

	#pragma HLS PIPELINE
	NextLinePrefetcher<NEXT_LINE_TYPES> nextLine = NextLinePrefetcher<NEXT_LINE_TYPES>();
	...
	if(!nop){
		nextLine(memoryBlockAddress_, blockAddressToPrefetch);
...
void RecursiveStridePrefetcherWithAXI(address_t inputAddress,
		axi_data_t *readPort,
		axi_data_t& prefetchedData,
		bool nop
		){
...
#pragma HLS PIPELINE
	...
	if(!nop){
		stride(memoryBlockAddress_ >> (REGION_BLOCK_SIZE_LOG2), memoryBlockAddress_, blockAddressToPrefetch);
		delta_t delta = blockAddressToPrefetch - memoryBlockAddress_;

		for(int i = 0; i < MAX_PREFETCHING_DEGREE; i++){
#pragma HLS UNROLL
			address_t addressToPrefetch = (((address_t)blockAddressToPrefetch) << BLOCK_SIZE_LOG2);
			blockAddressToPrefetch += delta;
