...
// #pragma HLS INLINE
	...
	if(true) {
		...
		if(true) {
			...
			DictionaryEntry < delta_t, dic_confidence_t > dictionaryEntry = this->dictionary.write(dictionaryEntries, delta, dictionaryClass);

			// 4) Fit-then-predict with the SVM:
			predictedClass = this->svm.fitAndPredict(weight_matrices, intercepts, inputBufferEntry.sequence, dictionaryClass);
...
#pragma HLS UNROLL
	...
	for (int k = 1; k < MAX_PREFETCHING_DEGREE; k++) {
#pragma HLS UNROLL
		if(k < numPredictions){
