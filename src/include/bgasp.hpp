...
public:
	...
	#pragma HLS ARRAY_PARTITION variable=dictionaryEntriesMatrix.entries complete
		...
		static BurstInputBufferEntriesMatrix<ib_tag_t, block_address_t, class_t, burst_length_t, ib_confidence_t, ib_lru_t>
				inputBufferEntriesMatrix = initBurstInputBufferEntries<ib_tag_t, block_address_t, class_t, burst_length_t, ib_confidence_t, ib_lru_t>();
	...
	#pragma HLS ARRAY_PARTITION variable=burstSvmMatrix.intercepts complete
		...
		static BurstInputBuffer<address_t, ib_index_t, ib_way_t, ib_tag_t, block_address_t, class_t, burst_length_t, ib_confidence_t, ib_lru_t> inputBuffer;
	...
	#pragma HLS PIPELINE
		...
		BurstInputBufferEntry<ib_tag_t, block_address_t, class_t, burst_length_t, ib_confidence_t, ib_lru_t> inputBufferEntryDummy;
		BurstInputBufferEntry<ib_tag_t, block_address_t, class_t, burst_length_t, ib_confidence_t, ib_lru_t> inputBufferEntry =
		...
		if (inputBufferEntry.lastAddress != memoryAddress) {
			...
			if (isInputBufferHit) {
				...
				if (inputBufferEntry.lastPredictedAddress == memoryAddress 
					&& inputBufferEntry.lastPredictedBurstLength == burstLength) {
				...
				burstSvm.recursivelyPredictAndFit(burstSvmMatrix.weightMatrices, burstSvmMatrixCopy.weightMatrices, burstSvmMatrix.intercepts, 
					burstSvmMatrixCopy.intercepts, inputBufferEntry.burstLengthSequence, burstLength,
	...
	#pragma HLS UNROLL
					...
					inputBufferEntry.burstLengthSequence[i] = inputBufferEntry.burstLengthSequence[i + 1];
				}
				inputBufferEntry.sequence[SEQUENCE_LENGTH - 1] = dictionaryClass;
				inputBufferEntry.burstLengthSequence[SEQUENCE_LENGTH - 1] = burstLength;
	...
	#pragma HLS UNROLL
					...
					inputBufferEntry.burstLengthSequence[i] = 0;
				}
				...
				predictedBurstLengths[0] = burstSvm.predict(burstSvmMatrixCopy.weightMatrices, burstSvmMatrixCopy.intercepts,
					inputBufferEntry.burstLengthSequence);
			...
			inputBufferEntry.lastPredictedBurstLength = predictedBurstLengths[0];
